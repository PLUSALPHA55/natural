...
export default function TimeChartClient({
  ...
  const loadShifts = async () => {
    ...
    console.log("🕒 shifts load result:", { shopId, date, data, error });
  ...
  const loadReservations = async () => {
    ...
    console.log("📅 reservations load result:", { shopId, date, data, error });
  ...
  return (
    <div className="w-full p-5">
      ...
      <div className="grid grid-cols-[150px_repeat(14,1fr)] border">
        ...
        {HOURS.map((h) => (
          <div key={h} className="border-r p-1 bg-yellow-50 text-center">
            {h}
          </div>
        ))}
      </div>
      ...
      {girls.map((girl) => (
        <div
          key={girl.id}
          className="grid grid-cols-[150px_repeat(14,1fr)] border border-t-0"
        >
